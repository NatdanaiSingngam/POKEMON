...
function itemList(me, battle = false) {
  const allowed = battle ? state.battle?.sides.includes(playerId) : state.players[state.turn]?.id === playerId && ['roll', 'shop'].includes(state.step) && !me.usedOutside;
...
function actionHtml() {
  ...
  else if (state.step === 'roll') body = `<p>ถึงตาคุณแล้ว ทอยเต๋าเพื่อเดินบนกระดาน</p>${state.lastRoll ? `<div class="big-die">🎲 ${state.lastRoll.value}</div>` : ''}<button class="btn primary block" data-act="roll">ทอยเต๋า</button>`;
