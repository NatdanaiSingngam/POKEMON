...
function detailsDockHtml() {
  ...
  return `<div class="detail-buttons"><button class="btn small" data-ui="team">ทีม ${me.pokemon.length}/6</button><button class="btn small" data-ui="quest">🏅 ${me.badges?.length || 0}/2 · ${quest ? `${quest.name} ${me.quest.progress}/${quest.target}` : 'ยังไม่มีเควส'}</button><button class="btn small" data-ui="log">เหตุการณ์</button></div>${openDetails === 'team' ? `<div class="detail-pop"><h2>ทีมโปเกมอน</h2><div class="monster-grid">${me.pokemon.map(mon => monCard(mon, false, me.badges?.length || 0, me.role)).join('')}</div></div>` : openDetails === 'quest' ? `<div class="detail-pop"><h2>เหรียญตราและเควส</h2><p>เหรียญตรา ${me.badges?.length || 0}/2 · 1 เหรียญใช้โปเกมอนโซนม่วงสู้ได้ · 2 เหรียญใช้โซนแดงและตำนานสู้ได้</p><p>${quest ? `${quest.name}: ${quest.text} (${me.quest.progress}/${quest.target}) · รางวัล ${quest.coins} เหรียญและ${quest.reward === 'item' ? 'ไอเทม' : 'โปเกมอน'}` : 'ยังไม่มีเควส ไปหยุดที่ช่องเควสเพื่อรับ'}</p>${quest && canAbandon ? '<button class="btn small" data-act="abandonQuest">ทิ้งเควส</button>' : ''}</div>` : openDetails === 'log' ? `<div class="detail-pop"><h2>เหตุการณ์ล่าสุด</h2><ol class="log-list">${state.log.map(line => `<li>${esc(line)}</li>`).join('')}</ol></div>` : ''}`;
}
function lobbyHtml() {
  const me = state.players.find(p => p.id === playerId);
